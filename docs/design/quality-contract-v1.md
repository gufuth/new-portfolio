...
> **STATUS: CURRENT / BINDING QUALITY LAW.** For current state and authority, use `../handoffs/2026-09-03-master-handoff-v8.md`. Historical handoff/path references below are subordinate to that current authority and `AGENTS.md`.
...
1. LOCKS (`../handoffs/2026-09-03-master-handoff-v8.md`, `filmframe-lock.md`, and current surface specs). Never relitigated silently. If a change genuinely requires touching a lock, STOP, name the lock, give the reason, and ask Ian. Read the reason before proposing the exception.
...
5. ONE-MOTION TEST: each surface has at most ONE ambient motion source (LANDING: none; WORK / MORE WORK: the single rare passing-road-light event family; ABOUT: none in the current honest implementation). Interaction feedback and short navigation cuts are exempt. Nothing loops decoratively, ever.
...
- MEASURE, DON'T EYEBALL. Sign footprints came from luminance-blob extraction, not visual guessing. Any geometry work: measure programmatically off the plate. The LANDING → WORK cut now follows this law by measuring the rendered WORK hotspot at click time.
- VERIFY RENDERED PIXELS. After every change to a live page: reload with cache-bust (?v=N — Chrome caches the dev server hard), screenshot or probe with JS, confirm the change is actually on screen. Source-level CI is necessary but does not substitute for the staged pixel pass.
...
- Pipeline is always: gpt-image-2 plate (blank surfaces where content will live) → _treat_film.py (the exact grain/grade/halation recipe — never a new recipe) → crop to the surface's locked geometry → composite content as live or layered elements → shared treatment on top. One generation never solves atmosphere and editable content at once.
- Renders land in the ACTIVE plan.json corridor, not this folder. Move them after.

## SOUND RULES
Sound is OFF by default and opt-in. Never autoplay sound on page load. The bell belongs to the diner door only. Synthesis over samples, tunables commented. A sound switch ships before the site does.

The current Ian-approved exception to the old "gesture/threshold only" wording is the WORK / MORE WORK **single rare passing-road event**: the visual event may occur after a long randomized wait; if and only if the visitor has already opted into sound and browser gesture policy permits it, that same physical event may include quiet tire/road sound. This is not an ambient loop and must never become frequent enough to read as one.

## COPY RULES
The register is dry against a haunted world; the contrast IS the voice. No AI-slop vocabulary. Diner idiom over app idiom. Case-study metadata never appears as decorative signage. The short in-world bio stays restrained. "The room is paid through Thursday" is the keeper line and the register benchmark.

## SESSION RITUALS
- OPEN: read `AGENTS.md`, the v8 handoff, then this contract, then inspect every relevant checked-in surface before touching anything. If a live host is available, verify it separately and say what was directly observed. State back to Ian what is locked and what the target is. Do not generate images by default.
- DURING: kill-list check on anything new; the seven tests before presenting; cache-bust every verification. If touching the tour runtime, keep `node --check tour-runtime.js` and `node scripts/verify-tour-runtime.mjs` green.
- CLOSE: update the handoff (new version if substantial), list every new file, name any lock added or challenged, and write down any miss that got caught.
