...
- A live desktop first-screen audit of all nine case routes at 1363×936 passed route loading, hero loading, headline/role/fact visibility, and horizontal-overflow checks. A source-level mobile/accessibility audit now enforces the index-first breakpoint, matching desktop/mobile cast, single-column cases, 44px navigation targets, reduced-motion/mobile ambient exclusions, focusable skip targets, and intrinsic case-image dimensions. Mobile rendered-pixel QA remains open. Alita and Cuervo have unusually tall source ratios; Selsun, Cuervo, Alita, and Atlantic include source images too small for final production and remain explicit media-replacement items.
